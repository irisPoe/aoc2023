...
class Day07Part2 : LinesPuzzle() {
    ...
    private class Hand(val hand: List<CardType>): Comparable<Hand> {
        ...
        private fun getHandType(cardOccurrences: List<CardOccurrence>): HandType {
            val highestAmountsOfCards = getHighestAmountsOfCards(cardOccurrences)
            return when (highestAmountsOfCards.highestAmount) {
                5 -> HandType.FIVE_OF_A_KIND
                4 -> HandType.FOUR_OF_A_KIND
                3 -> {
                    if (highestAmountsOfCards.secondHighestAmount == 2) {
                        HandType.FULL_HOUSE
                    } else {
        ...
        private fun getHighestAmountsOfCards(cardOccurrences: List<CardOccurrence>): HighestAmoundOfCardsResult {
            val sortedCardOccurrences = cardOccurrences.sortedByDescending { it.amount }
            val numberOfJokers = sortedCardOccurrences.firstOrNull { it.card == CardType.CardJ }?.amount ?: 0

            if (sortedCardOccurrences[0].amount == 5) {
                return HighestAmoundOfCardsResult(5, 0)
            }

            val highestAmount = if (sortedCardOccurrences[0].card == CardType.CardJ) {
                sortedCardOccurrences[1].amount + sortedCardOccurrences[0].amount
            } else {
                sortedCardOccurrences[0].amount + numberOfJokers
            }
            ...
            val secondHighestAmount =
                if (sortedCardOccurrences[0].card == CardType.CardJ || sortedCardOccurrences[1].card == CardType.CardJ) {
                    sortedCardOccurrences[2].amount
                } else {
                    sortedCardOccurrences[1].amount
                }

            return HighestAmoundOfCardsResult(highestAmount, secondHighestAmount)
        }
        ...
        private class HighestAmoundOfCardsResult(val highestAmount: Int, val secondHighestAmount: Int)
    }
